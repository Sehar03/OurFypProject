...
const SharedFoodCard = ({ item, navigation, currentOrderRoute }) => {
  ...
  useEffect(() => {
    ...
    const selectedTime = moment(`${item.productSelectedDate} ${item.productSelectedTime}`, 'MMM DD, YYYY h:mm A');
    ...
    if (currentDate.isSameOrAfter(selectedDate) && currentDate.isSameOrAfter(selectedTime)) {
      ...
      <Neomorph
          ...
          <View
            ...
            <View style={{ flexDirection: 'row' }}>
              ...
              <TouchableOpacity onPress={() => {
                ...
                <Image source={require('../../assets/Images/detailIcon.png')} style={{ height: hp('3.5'), width: wp('7') }} />
              </TouchableOpacity>
            )
            }
        ...
        <View
            ...
            <View style={{ flexDirection: "row", alignSelf: "center" }}>
              <Text style={{ color: "green", fontSize: wp('4'), fontFamily: "Poppins-SemiBold" }}>Share Food Status : </Text>
              <Text style={{ color: item.status === 'Order Delivered' ? 'green' : 'firebrick', marginLeft: wp('1.5'), fontSize: wp('4'), fontFamily: "Poppins-SemiBold" }}>{item.status}</Text>
